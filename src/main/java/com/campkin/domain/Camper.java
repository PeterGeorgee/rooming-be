...
@Entity @Table(name="campers") @Getter @Setter @NoArgsConstructor public class Camper {
 @Id private UUID id=UUID.randomUUID(); @ManyToOne(optional=false) @JoinColumn(name="camp_id") private Camp camp; @Column(nullable=false) private String name; @Column(name="normalized_name",nullable=false) private String normalizedName; @Enumerated(EnumType.STRING) @Column(nullable=false) private Domain.Gender gender; @Column(nullable=false) private LocalDate birthdate; @ManyToOne @JoinColumn(name="room_id") private Room room; @ManyToOne @JoinColumn(name="discussion_group_id") private DiscussionGroup discussionGroup; @Version private long version;
